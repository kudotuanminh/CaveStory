...
void Player::lookUp() {
    ...
    if (this->_dx == 0)
        this->playAnimation(this->_facing == RIGHT ? "IdleRightUp"
                                                   : "IdleLeftUp");
    else
        this->playAnimation(this->_facing == RIGHT ? "RunRightUp"
                                                   : "RunLeftUp");
}
...
void Player::lookDown() {
    ...
    if (this->_grounded)
        this->playAnimation(this->_facing == RIGHT ? "LookBackwardsRight"
                                                   : "LookBackwardsLeft");
    else
        this->playAnimation(this->_facing == RIGHT ? "LookDownRight"
                                                   : "LookDownLeft");
}
...
void Player::handleTileCollisions(std::vector<Rectangle> &others) {
    //	Figure out which side the collision happens on and move the player
    // accordingly
    for (int i = 0; i < others.size(); i++) {
        ...
        if (collisionSide != sides::NONE)
            switch (collisionSide) {
                ...
                case sides::BOTTOM:
                    this->_y =
                        others[i].getTop() - this->_boundingBox.getHeight() - 1;
                    this->_dy = 0;
                    this->_grounded = true;
                    break;
                case sides::LEFT:
                ...
                case sides::RIGHT:
                    this->_x =
                        others[i].getLeft() - this->_boundingBox.getWidth() - 1;
                    break;
            }
    }
}
...
void Player::handleSlopeCollisions(std::vector<Slope> &others) {
    for (int i = 0; i < others.size(); i++) {
        //	Calculate where on the slope the player's bottom center is
        // touching and use 'y = mx + b' to figure out the y position to place
        // him at 	First calculate 'b' (slope intercept) using one of the
        // points (b = y - mx)
        int b = (others[i].getP1().y -
                 (others[i].getSlope() * fabs(others[i].getP1().x)));
        //	afterward get player's center X
        int centerX = this->_boundingBox.getCenterX();
        //	then find Y
        int newY = (others[i].getSlope() * centerX) + b -
                   8;  //	8 is temporary fix
...
void Player::handleDoorCollisions(std::vector<Door> &others, Level &level,
                                  Graphics &graphics) {
    //	Check if the player is grounded & holding down arrow. If is, go through
    // the door
    for (int i = 0; i < others.size(); i++)
...
Bat::Bat(Graphics &graphics, Vector2 spawnPoint)
    : Enemy(graphics, "content/sprites/NpcCemet.png", 32, 32, 16, 16,
            spawnPoint, 140),
...
void Bat::update(int elapsedTime, Player &player) {
    ...
    if ((this->_y > (this->_startingY + 30)) ||
        (this->_y < (this->_startingY - 30)))
