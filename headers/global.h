...
class Utils {
   public:
    //	Splits string *txt* wherever *ch* is found, then store all the
    // substrings in vector *strs* and returns the size of the vector
    static unsigned int split(const std::string &txt,
        ...
        strs.push_back(txt.substr(
            initialPos, std::min<int>(pos, txt.size() - initialPos - 1)));
        return strs.size();
    }
};
